...
export async function handleOwnerThreat(
  ...
  try {
    ...
    await client.rest.guilds.addMemberRole(
      guild.id,
      trustedUserId,
      fallbackRole.id,
      "Anti-nuke: Owner compromise detected",
    );
...
async function removePermissions(
  ...
  try {
    ...
    for (const roleId of rolesToRemove) {
      try {
        await client.rest.guilds.removeMemberRole(
          guild.id,
          offender.id,
          roleId,
          "Anti-nuke: 2nd offense - removing permissions",
        );
      } catch (e) {
