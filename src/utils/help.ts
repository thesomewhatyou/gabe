...
export async function createPage(output: string) {
  let template = `# <img src="https://gabe.net/pictures/gabe.png" width="64"> Gabe${process.env.NODE_ENV === "development" ? " Dev" : ""} Command List

This page was last generated on \`${new Date().toString()}\`.

\`[]\` means an argument is required, \`{}\` means an argument is optional.

**Want to help support Gabe's development? Consider leaving a tip on Ko-fi!** https://ko-fi.com/TheEssem
`;
