...
function getProcesses(): Promise<ProcessDescription[]> {
  return new Promise((resolve, reject) => {
    pm2.list((err, list) => {
      if (err) reject(err);
      resolve(list.filter((v) => v.name?.includes("Gabe-proc")));
    });
  });
}
...
if (process.env.METRICS && process.env.METRICS !== "") {
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    if (reqUrl.pathname === "/" || reqUrl.pathname === "/metrics") {
      res.write(`# HELP gabe_command_count Number of times a command has been run
# TYPE gabe_command_count counter
`);
      if (database) {
        const counts = await database.getCounts(true);
        for (const [i, w] of counts.entries()) {
          res.write(`gabe_command_count{command="${i}"} ${w}\n`);
        }
      }

      res.write(`# HELP gabe_servers Number of servers/guilds the bot is in
# TYPE gabe_servers gauge
gabe_servers ${serverCount}
# HELP gabe_shards Number of shards the bot has
# TYPE gabe_shards gauge
gabe_shards ${shardData.length}
# HELP gabe_shard_ping Latency of each of the bot's shards
# TYPE gabe_shard_ping gauge
`);

      for (const shard of shardData) {
        if (shard.latency)
          res.write(`gabe_shard_ping{shard="${shard.id}", status="${shard.status}"} ${shard.latency}\n`);
      }
...
logger.info("Started Gabe management process.");
...
function awaitStart(i: number, shardArrays: number[][]): Promise<void> {
  return new Promise((resolve) => {
    pm2.start(
      ...
      (err) => {
        if (err) {
          logger.error(`Failed to start Gabe process ${i}: ${err}`);
          process.exit(0);
        } else {
          logger.info(`Started Gabe process ${i}.`);
          resolve();
        }
      },
    );
