...
Here are some instructions to get Gabe up and running from source.
...
If you have any further questions regarding setup, feel free to ask in the #support channel on the [Gabe Support server](https://gabe.net/support).

!!! tip

    You can run the bot using Docker or Podman for a somewhat simpler setup experience. [Click here to go to the container setup guide.](https://docs.gabe.net/containers)
...
Node.js is the runtime that Gabe is built on top of. The bot requires version 22.0.0 or above to run.
...
Gabe uses the pnpm package manager to manage dependencies and run build scripts. You can use Corepack (a tool included with Node.js) to enable it, and once you attempt to run it for the first time it will ask you to install it:
...
Gabe officially supports two database systems: SQLite and PostgreSQL. While SQLite is smaller and requires no initial setup, PostgreSQL has better performance (especially in large environments).
...
If you would like to use the PostgreSQL database, view the setup instructions [here](https://docs.gabe.net/postgresql) and come back here when you're finished.
...
Lavalink is the audio server used by Gabe for music playback. If you do not plan on using this feature, you can safely skip this step.
...
Gabe requires Lavalink version v4 or later, which requires a Java (17 or later) installation. Choose the OS you're using below for the command to use to install Java.
...
An overview of each of the variables in the `.env` file can be found [here](https://docs.gabe.net/config).
...
If you have any further questions regarding self-hosting, feel free to ask in the #support channel on the [Gabe Support server](https://gabe.net/support).
