...
class AntinukeStatusCommand extends Command {
  async run() {
    ...
    const whitelistedUsers =
      settings.whitelisted_users.length > 0 ? settings.whitelisted_users.map((id) => `<@${id}>`).join(", ") : "None";

    const whitelistedRoles =
      settings.whitelisted_roles.length > 0 ? settings.whitelisted_roles.map((id) => `<@&${id}>`).join(", ") : "None";

    const trustedUser = settings.trusted_user
      ? settings.trusted_user.startsWith("<")
        ? settings.trusted_user
        : `<@${settings.trusted_user}>`
      : "Not configured";

    const logChannel = settings.log_channel_id ? `<#${settings.log_channel_id}>` : "Not configured";
